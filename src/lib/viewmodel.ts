...
export function toPdfOffer(state: OfferEditorState, company: CompanySettings | null): PdfOffer {
  return {
    ...
    company: {
      ...
      cif: company?.cif ?? 'RO42820863',
      reg_number: company?.reg_number ?? 'EUID ROONRC.J2020008572404',
      address: company?.address ?? 'Str. Câmpineanca nr. 8, Sector 3, București',
      phone: company?.phone ?? '0726.425.618',
      email: company?.email ?? 'corina@andcorauto.com',
